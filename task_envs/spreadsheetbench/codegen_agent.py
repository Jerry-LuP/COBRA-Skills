...
from cobras.cobras_core.model.openai import (
    get_target_chat_extra_body,
    get_reasoning_effort,
    get_target_client,
    _needs_responses_api,
    tracker,
)
from cobras.cobras_core.model import get_target_deployment, is_target_exec_backend
...
from cobras.task_envs.spreadsheetbench.evaluator import evaluate
...
def _prepare_exec_workspace(
    *,
    ...
    diagnostic_instruction: str,
    diagnostic_trace_context: str,
    workspace_name: str = "exec_single",
    ...
    prepare_workspace(
        ...
        skill_md=skill_md,
        task_text=task_md,
        extra_files={"run_solution.py": _build_exec_driver()},
...
def _chat_call(
    ...
    if _needs_responses_api(deployment):
        ...
        if hasattr(resp, "usage") and resp.usage:
            ...
            record_token_usage(
                role="student", model=deployment, stage="spreadsheet_rollout", usage=resp.usage
            )
...
def run_single(
    ...
    if is_target_exec_backend():
        ...
        work_dir, skill_md, task_md, prompt = _prepare_exec_workspace(
...
def run_multi(
    ...
    if is_target_exec_backend():
        deadline = None if no_task_timeout else time.time() + task_timeout
        deployment = _get_deployment()
        work_dir, skill_md, task_md, initial_prompt = _prepare_exec_workspace(
            instruction=instruction,
            input_xlsx=input_xlsx,
            output_path=output_path,
            instruction_type=instruction_type,
            ...
            if not code.strip():
                ...
                if ok:
                    if gold_path and answer_position:
                        from cobras.task_envs.spreadsheetbench.rollout import _auto_verify_output
