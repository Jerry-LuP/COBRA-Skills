...
# Derived from SpreadsheetBench and modified for COBRA-Skills.
...
def _parse_range(range_str: str):
    ...
    return (
        (_col_name2num(sc), int(sr) if sr else None),
        (_col_name2num(ec), int(er) if er else None),
    )


def _generate_cell_names(range_str: str, max_row: int | None = None):
    if ":" not in range_str:
        return [range_str]
    (sc, sr), (ec, er) = _parse_range(range_str)
    if sr is None and er is None and max_row is not None:
        sr = 1
        er = max_row
    elif sr is None or er is None:
        raise ValueError(f"Incomplete row range: {range_str}")
...
def _cell_level_compare(wb_gt, wb_proc, sheet_name: str, cell_range: str):
    ...
    for cn in _generate_cell_names(cell_range, max_row=max_row):
        cg = ws_gt[cn]
        cp = ws_proc[cn]
