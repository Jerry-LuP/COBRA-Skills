...
from cobras.task_envs.searchqa.evaluator import evaluate
from cobras.cobras_core.model import chat_target, get_target_deployment, is_target_exec_backend
    ...
    try:
        ...
        if is_target_exec_backend():
            ...
            for turn in range(max_turns):
                response, raw, system, user = _run_exec_once(
                    pred_dir=pred_dir,
        ...
        if is_timeout_error(exc):
            ...
            return result
        raise_evaluation_error("SearchQA", item_id, exc)
    ...
    if os.path.exists(results_path):
        ...
        with open(results_path) as f:
            for line in f:
                try:
                    r = json.loads(line)
                    latest_by_id[str(r["id"])] = r
                except Exception:
                    pass

        # Correct/incorrect agent responses and explicit timeouts are terminal.
        # Historical infrastructure-error rows are discarded and rerun.
        existing = [
            r
            for r in latest_by_id.values()
            if is_valid_result_row(r)
        ]
                ...
                for fut in done:
                    ...
                    try:
                        ...
                        if is_timeout_error(exc):
                            ...
                            raise_evaluation_error("SearchQA", item["id"], exc)
                    results.append(res)
                    completed += 1
                    if res.get("hard", 0):
                        correct_count += 1
