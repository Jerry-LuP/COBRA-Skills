...
def _worker_loop(cmd_q, result_q, config, seed, is_train, eval_dataset, gamefile):
    ...
    try:
        # ALFWorld prints its environment banner, dataset counts, and a tqdm
        # bar while every process initializes. They add a large amount of
        # noise when episodes are scheduled one at a time, while startup
        # failures are already returned to the parent with their traceback.
                ...
                if gamefile:
                    ...
                    if not is_train:
                        split_key = (
                            ...
                            else 'eval_ood_data_path'
                        )
                    worker_config['dataset'][split_key] = os.path.dirname(gamefile)

                env_type = worker_config['env']['type']
                base_env = get_environment(env_type)(
                    worker_config,
                    train_eval='train' if is_train else eval_dataset,
                )
                worker = AlfworldWorker(worker_config, seed, base_env, gamefile)
