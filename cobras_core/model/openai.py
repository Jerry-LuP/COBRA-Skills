"""OpenAI and OpenAI-compatible backend with token tracking.

Provides optimizer/target dual-model chat functions for OpenRouter, Yunwu,
custom endpoints, local vLLM, and Azure OpenAI.
"""
...
from cobras.cobras_core.token_usage import record_token_usage
...
YUNWU_APP_CODE = os.environ.get("YUNWU_APP_CODE") or os.environ.get("APP_CODE") or ""
...
def _make_client(role: str) -> AzureOpenAI | OpenAI:
    ...
    if auth_mode in {"openai_compatible", "compat", "openai"}:
        default_headers = {"User-Agent": "COBRAS"}
        app_code = _yunwu_app_code()
        if app_code and _is_yunwu_endpoint(cfg["endpoint"]):
...
def _is_yunwu_endpoint(endpoint: str | None = None) -> bool:
    text = str(endpoint or ENDPOINT or "").lower()
    return "yunwu.ai" in text


def _yunwu_app_code() -> str:
    return (
        os.environ.get("YUNWU_APP_CODE")
        or os.environ.get("APP_CODE")
        or YUNWU_APP_CODE
        or ""
    ).strip()


def _using_yunwu(client: AzureOpenAI | OpenAI | None = None) -> bool:
    base_url = str(getattr(client, "base_url", "") or "")
    return _is_yunwu_endpoint(base_url)


def _chat_max_tokens_kwargs(
    *,
    client: AzureOpenAI | OpenAI,
    max_completion_tokens: int,
) -> dict[str, Any]:
    if _using_yunwu(client):
        return {"max_tokens": max_completion_tokens}
    return {"max_completion_tokens": max_completion_tokens}


def get_target_chat_extra_body() -> dict[str, Any]:
    """Build local chat-template options for target Chat Completions calls."""

    raw = os.environ.get("OPENAI_CHAT_TEMPLATE_ENABLE_THINKING", "").strip().lower()
    if not raw:
        return {}
    if raw in {"1", "true", "yes", "on"}:
        ...
        raise ValueError(
            "OPENAI_CHAT_TEMPLATE_ENABLE_THINKING must be a boolean, "
            f"got {raw!r}"
        )
    return {"chat_template_kwargs": {"enable_thinking": enabled}}


def _chat_extra_kwargs(client: AzureOpenAI | OpenAI) -> dict[str, Any]:
    kwargs: dict[str, Any] = {}
    if _using_yunwu(client):
        app_code = _yunwu_app_code()
        if app_code:
            kwargs["extra_headers"] = {"APP-Code": app_code}
    if str(getattr(client, "base_url", "")).lower().startswith(
        ("http://127.0.0.1", "http://localhost")
    ):
        extra_body = get_target_chat_extra_body()
        if extra_body:
    ...
    for attempt in range(retries):
        try:
            if _needs_responses_api(deployment):
                ...
                actual_effort = _normalize_reasoning_effort(reasoning_effort) or REASONING_EFFORT
                if actual_effort:
                ...
                kwargs: dict[str, Any] = dict(
                    ...
                    **_chat_max_tokens_kwargs(
                        client=client,
                        max_completion_tokens=max_completion_tokens,
                    ),
                    **_chat_extra_kwargs(client),
                )
                raw_temperature = os.environ.get("OPENAI_TARGET_TEMPERATURE", "").strip()
                if raw_temperature:
                    kwargs["temperature"] = float(raw_temperature)
                actual_effort = _normalize_reasoning_effort(reasoning_effort) or REASONING_EFFORT
                if actual_effort is not None and not _using_yunwu(client):
            ...
            print(
                f"[llm-retry] stage={stage} model={deployment} "
                f"attempt={attempt + 1}/{retries} sleep={sleep}s error={e!r}",
                flush=True,
            )
            time.sleep(sleep)
        ...
        try:
            if _needs_responses_api(deployment):
                ...
                if tools:
                    ...
                    if actual_effort is not None and _using_yunwu(client):
                        kwargs["reasoning_effort"] = actual_effort
            ...
            sleep = min(2 ** attempt, 30)
            print(
                f"[llm-retry] stage={stage} model={deployment} messages=1 "
                f"attempt={attempt + 1}/{retries} sleep={sleep}s error={e!r}",
                flush=True,
            )
            time.sleep(sleep)
...
def chat_optimizer(
    ...
    record_token_usage(role="teacher", model=OPTIMIZER_DEPLOYMENT, stage=stage, usage=result[1])
    return result
...
def chat_with_deployment(
    ...
    record_token_usage(role="teacher", model=deployment, stage=stage, usage=result[1])
    return result
...
def chat_target(
    ...
    record_token_usage(role="student", model=TARGET_DEPLOYMENT, stage=stage, usage=result[1])
    return result
...
def _normalize_reasoning_effort(effort: str | None) -> str | None:
    ...
    if normalized in {"", "none", "off", "false", "no", "0"}:
        return None
    return normalized


def set_reasoning_effort(effort: str | None) -> None:
    """Set reasoning effort for all LLM calls. None/off/none = off."""
